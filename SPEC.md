...
- **master/main 分支对齐**：本地默认分支是 `master`，GitHub 默认是 `main`。Vercel 监听 `master` 分支。修复已同步到两分支，确保 Vercel 自动部署链路正常。

**Vercel 部署地址**：`https://ip-website-generator-saas.vercel.app`

**Git 分支说明**：
- `master` — 本地开发分支，修复先提交到这里
- `main` — GitHub 默认分支，Vercel 监听的是 `master`
- 两者现在同步，任何 push 到 master 会触发 Vercel 自动部署
